...
function loadConfig() {
  ...
  try {
    ...
    if (merged.openadapter_base_url && merged.openadapter_base_url.includes('api.openadapter.ai')) {
      merged.openadapter_base_url = merged.openadapter_base_url.replace('api.openadapter.ai', 'api.openadapter.in');
    }
    return merged;
  } catch (err) {
    return { ...DEFAULT_CONFIG };
