...
const TokenOptimizer = {
  ...
  cleanStackTrace(text) {
    if (typeof text !== "string" || (!text.includes("    at ") && !text.includes('File "'))) return text;
    const lines = text.split("\n");
    const filtered = lines.filter(l => {
      if (l.includes("node:internal/") || l.includes("node_modules/bun/") || l.includes("node_modules/electron/")) return false;
      if (l.includes("/usr/lib/python3") && l.includes("/urllib/")) return false;
      return true;
    });
  ...
  compressToolOutput(output, maxLines = 40, maxChars = 10000) {
    if (typeof output !== "string") output = String(output || "");
    output = this.stripAnsi(output);
    output = this.cleanStackTrace(output);
    output = this.compactJson(output);

    const isListing = /^(?:total \d+|drwx|\.\/|-rw-)/m.test(output) && output.includes("\n");
    if (isListing) maxLines = Math.min(maxLines, 20);

    const isProcList = /^(?:USER\s+PID|UID\s+PID|\s*PID\s+TTY)/m.test(output);
    if (isProcList) maxLines = Math.min(maxLines, 15);

    const isDiff = output.startsWith("diff --git") || output.startsWith("--- a/") || output.startsWith("@@ ");
    if (isDiff) maxLines = Math.min(maxLines, 60);
    ...
    if (lines.length > maxLines) {
      ...
      output = head + "\n\n... [" + omitted + " lines omitted via DTOC Token Protocol] ...\n\n" + tail;
    }
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (typeof m.content === "string") {
        ...
        if (isOldTurn && (m.role === "tool" || m.content.startsWith("[Tool Output:"))) {
          const fileMatch = m.content.match(/(?:File Path:|cat |sed -n) `?([^`\n]+)`?/i);
          if (fileMatch) {
            const fileName = fileMatch[1].trim();
            if (seenFileReads.has(fileName)) {
              m.content = "[File read: " + fileName + " (cached from earlier turn)]";
            } else {
              seenFileReads.add(fileName);
            }
          }

          if (m.content.length > 1200) {
            m.content = m.content.slice(0, 500) + "\n... [Historical tool output summarized for token efficiency] ...\n" + m.content.slice(-250);
          }
        }
      }
      optimized.push(m);
...
async function normalizeTargetModel(modelName, defaultModel = "0G-DeepSeek-v4-Flash") {
  if (!modelName || modelName === "auto") return defaultModel;
  let clean = modelName.trim();
  if (clean.startsWith("openadapter/")) clean = clean.substring("openadapter/".length);
  const lower = clean.toLowerCase();
  
  const map = {
    "deepseek-v4-flash:dev": "0G-DeepSeek-v4-Flash",
    "deepseek-v4-flash:free": "0G-DeepSeek-v4-Flash",
    "deepseek-v4-flash": "0G-DeepSeek-v4-Flash",
    "deepseek-v4-pro": "0G-DeepSeek-v4-Pro",
    "glm-5.2:dev": "0G-GLM-5.2",
    "glm-5.2": "0G-GLM-5.2",
    "gpt-oss-120b:dev": "0G-GLM-5.2",
    "qwen3.6-27b:dev": "0G-Qwen3.6",
    "gemma-4-31b-it:dev": "0G-Qwen3.7-max",
    "mimo-v2.5:dev": "0g-minimax-m3",
    "kimi-k2.6:dev": "0g-minimax-m3",
    "minimax-m2.7:dev": "0g-minimax-m3"
  };
  if (map[lower]) return map[lower];
  if (lower.includes("deepseek") && lower.includes("pro")) return "0G-DeepSeek-v4-Pro";
  if (lower.includes("deepseek")) return "0G-DeepSeek-v4-Flash";
  if (lower.includes("qwen") && (lower.includes("3.7") || lower.includes("max"))) return "0G-Qwen3.7-max";
  if (lower.includes("qwen")) return "0G-Qwen3.6";
  if (lower.includes("minimax") || lower.includes("kimi") || lower.includes("mimo")) return "0g-minimax-m3";
  if (lower.includes("glm")) return "0G-GLM-5.2";
  return clean;
}

function handleResponsesApi(req, res, config) {
  let bodyStr = "";
  ...
  req.on("end", async () => {
    try {
      ...
      const targetModel = normalizeTargetModel(payload.model, config.default_model);
  ...
  if (Array.isArray(payload.input)) {
    for (const item of payload.input) {
      ...
      if (item.type === "message" || item.role) {
        ...
        rawMessages.push({
          role: item.role || "user",
          content: TokenOptimizer.normalizeWhitespace(TokenOptimizer.stripAnsi(content.trim()))
        });
        ...
        const compressedOut = TokenOptimizer.compressToolOutput(item.output || "");
        rawMessages.push({
          role: "user",
          content: "[Tool Output: " + compressedOut + "]"
        });
      }
    }
  }
